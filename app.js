...
function fmtIndex(value) { 
  return (!value || !isFinite(value)) ? "-" : Number(value).toFixed(2); 
}
...
// 층 정렬 로직 (B2F -> B1F -> 1F -> 2F)
function parseFloor(f) {
  if (!f) return { type: 'unknown', num: 0, raw: f, label: f };
  const m = f.match(/\d+/);
  const num = m ? parseInt(m[0], 10) : 0;
  if (f.includes("지하") || f.toUpperCase().includes("B")) {
    return { type: 'B', num: num, raw: f, label: `B${num}F` };
  }
  if (f.includes("지상") || f.includes("층") || f.toUpperCase().includes("F")) {
    return { type: 'F', num: num, raw: f, label: `${num}F` };
  }
...
function parseSheetEntries(rows, projectKey, fileName, sheetName) {
  ...
  for (let r = 4; r < rows.length; r++) {
    ...
    let possibleFloor = normalizeDisplayText(row1[2]) || normalizeDisplayText(row1[1]) || normalizeDisplayText(row1[0]);

    if (possibleFloor) {
      let currentFloor = possibleFloor;
      if (currentFloor === "계" || currentFloor.includes("합계")) {
        currentFloor = "합계";
      }
      ...
      for (let c = colStart; c <= colEnd; c++) {
        ...
        if (name3 && !skipHeaders.includes(name3)) {
          ...
          if (val !== 0) {
            entries.push({ projectKey, building: buildingName, floor: currentFloor, rawName: name3, normalizedName: normalizeText(name3), qty: val });
          }
        }
      }

      // 두 번째 줄 매칭 (병합된 경우 4행 타이틀)
      if (r + 1 < rows.length) {
        let row2 = rows[r + 1] || [];
        let nextPossibleFloor = normalizeDisplayText(row2[2]) || normalizeDisplayText(row2[1]) || normalizeDisplayText(row2[0]);
        
        if (!nextPossibleFloor) {
          for (let c = colStart; c <= colEnd; c++) {
            let name4 = normalizeDisplayText(headerRow4[c]);
            if (name4 && !skipHeaders.includes(name4)) {
              let val = toNumber(row2[c]);
              if (val !== 0) {
                entries.push({ projectKey, building: buildingName, floor: currentFloor, rawName: name4, normalizedName: normalizeText(name4), qty: val });
              }
            }
          }
          r++; // 병합된 행 점프
...
function getCategoryClass(category) {
  if (category === "콘크리트") return "is-concrete"; 
  if (category === "거푸집") return "is-form"; 
  if (category === "철근") return "is-rebar"; 
  if (category === "잡") return "is-exclude"; 
  return "is-exclude";
}
...
function renderTypeahead(items) {
  ...
  if (!items.length) { 
    typeaheadRoot.innerHTML = `<div class="typeahead-empty">일치하는 항목이 없습니다.</div>`; 
    return; 
  }
...
function moveTypeahead(delta) {
  ...
  if (next < 0) state.typeahead.activeIndex = len - 1; 
  else if (next >= len) state.typeahead.activeIndex = 0; 
  else state.typeahead.activeIndex = next;
...
function calcCompareRows() {
  ...
  // 4개 중분류 강제 정렬
  const categories = ["콘크리트", "거푸집", "철근", "잡"]; 
  const rows = [];
...
function renderCompareTable(data) {
  ...
  let thHtml = `<tr><th>동</th><th>중분류</th>`;
...
function exportCompareExcel() {
  if (!state.lastData || !state.lastData.rows.length) { 
  ...
  for (let c = 0; c < headers.length; c++) {
    ws[XLSX.utils.encode_cell({ c: c, r: 0 })] = { v: headers[c], t: "s", s: headerStyle };
  }
  rowsFormat[0] = { hpt: 25 };
...
function resetAll() {
  ...
  theadEl.innerHTML = `<tr><th>동</th><th>중분류</th><th>층평균</th><th>3개프로젝트평균</th><th>Index(현재/평균)</th></tr>`;
...
for (const pk of PROJECT_KEYS) {
  fileInputs[pk].addEventListener("change", updateFileListText);
}
...
btnCalc.addEventListener("click", () => { 
  try { 
    if (mappingModal.classList.contains("is-open")) saveMappingFromUI(); 
    validateBeforeCalc(); 
    const data = calcCompareRows(); 
    renderCompareTable(data); 
    btnExportCsv.disabled = data.rows.length === 0; 
    setStatus("비교표 생성 완료"); 
  } catch (error) { 
    console.error(error); 
    setStatus("오류 발생"); 
...
document.addEventListener("click", (e) => { 
  if (!typeaheadRoot.hidden) { 
    ...
    if (!clickedInsideTypeahead && !clickedInput) hideTypeahead(); 
  } 
});

window.addEventListener("resize", () => { 
  if (!typeaheadRoot.hidden && state.typeahead.targetInput) positionTypeahead(state.typeahead.targetInput); 
});
